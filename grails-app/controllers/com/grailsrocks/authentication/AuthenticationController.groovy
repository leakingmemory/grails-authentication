...
class AuthenticationController {
    ...
    def index = {
        ...
        AuthenticatedUser user = session[AuthenticationService.SESSION_KEY_AUTH_USER]
    ...
    def login = { LoginForm form ->
        ...
        if (log.debugEnabled) log.debug("LoginEmail ${form.email}, ${form.password}")

        def urls = extractParams()
        if (!form.hasErrors()) {
            def loginResult = authenticationService.loginWithEmail(form.email, form.password)
            if (loginResult.result == 0) {
                flash.loginForm = form
                if (log.debugEnabled) log.debug("Login succeeded for [${form.email}]")
                redirect(flash.authSuccessURL ? flash.authSuccessURL : urls.success)
            } else {
                flash.loginForm = form
                flash.authenticationFailure = loginResult
                if (log.debugEnabled) log.debug("Login failed for [${form.email}] - reason: ${loginResult.result}")
                redirect(flash.authFailureURL ? flash.authFailureURL : urls.error)
            }
        } else {
            flash.loginForm = form
            flash.loginFormErrors = form.errors // Workaround for grails bug
            if (log.debugEnabled) log.debug("Login failed for [${form.email}] - form invalid: ${form.errors}")
            redirect(flash.authErrorURL ?: urls.error)
        }
    }
